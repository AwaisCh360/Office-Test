...
import PRReviewsView from "@/components/PRReviewsView";
import MembersView from "@/components/MembersView";
import { ProfileView } from "@/components/ProfileView";
import { checkSession } from "@/data/serverSource";

export type View = "dashboard" | "new_scan" | "pr_reviews" | "overview" | "issues" | "agents" | "history" | "email" | "feedback" | "admin" | "members" | "profile";
...
export default function App() {
  ...
  }, [refreshAuth, refreshRuns, isLoggedIn]);
  ...
  const selectRun = useCallback((name: string) => {
    setActiveRun((prev) => {
      ...
      // We are selecting a new run, clear the old one.
      setSelectedId(null);
      setRun(null);
      setError(null);
      initialViewAppliedRef.current = false;
  ...
  if (isLoggedIn === null) {
    return <div className="flex min-h-screen items-center justify-center bg-black"><Loader2 className="h-6 w-6 animate-spin text-emerald-500" /></div>;
  }
        ...
        <div className="max-w-[88rem] mx-auto px-3 sm:px-6 py-8 sm:py-12 space-y-6">
          ...
          {view === "dashboard" ? (
            ...
            <MembersView 
              ...
              isCurrentUserAdmin={auth?.is_admin ?? false}
            />
          ) : !run && !error ? (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-white">Download PDF report of this run</p>
          <p className="mt-0.5 text-xs text-[#888]">
            Generates a local PDF containing all vulnerabilities.
          </p>
        </div>
