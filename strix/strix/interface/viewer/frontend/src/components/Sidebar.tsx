...
export default function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "sticky top-0 z-20 hidden h-screen flex-shrink-0 flex-col overflow-hidden border-r border-white/10 bg-[#09090b] lg:flex",
      ...
      >
          ...
          <div className="flex flex-row py-1 px-2">
            <div className="flex h-10 w-full flex-row items-center">
              <a
                ...
                className="flex flex-1 flex-row items-center gap-2 rounded-md py-2 pl-2.5 pr-1 min-w-0 transition-colors hover:bg-white/5"
              ...
              >
                ...
                <span className="flex flex-1 flex-row items-center gap-2 min-w-0">
                  <span className="truncate min-w-0 text-[14px] font-medium text-white">Strix</span>
                  <span className="flex h-5 flex-shrink-0 items-center rounded px-2 text-[11px] font-medium text-zinc-400 bg-white/5">
                    Local
                  </span>
                </span>
              </a>
              <a
                ...
                rel="noopener noreferrer"
                onClick={() => trackCta("logo", "sidebar")}
                className="flex flex-none items-center rounded-md px-1.5 py-2 transition-colors hover:bg-white/5"
        ...
        <nav className="relative min-w-[160px] flex-1 overflow-y-auto overflow-x-clip scrollbar-thin pb-10 pt-2">
          <div className="relative flex flex-col gap-px px-2">
            ...
            <NavItem
              ...
              label="New Scan"
              active={view === "new_scan"}
              onClick={() => onSelectView("new_scan")}
            />
            <NavItem
            ...
            <hr className="mx-0 my-1 h-px w-full border-0 bg-white/10" />
          ...
          <div className="relative p-2">
            {verified && email ? (
              <button
                ...
                className="relative flex w-full cursor-pointer items-center gap-2 rounded-md bg-transparent px-2.5 py-2 transition-colors hover:bg-white/5"
              >
                <span
                  className="flex flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-emerald-400 to-cyan-500"
                  style={{ width: 20, height: 20 }}
                ...
                <span className="flex min-w-0 flex-1 flex-col text-left">
                  ...
                  <span className="truncate text-[11px] text-zinc-400">Linked to this machine</span>
                </span>
              </button>
            ) : (
              <div className="flex items-center gap-2 rounded-md px-2.5 py-2">
                ...
                <span className="flex min-w-0 flex-1 flex-col text-left">
                  <span className="truncate text-[13px] font-medium text-white">Local viewer</span>
                </span>
              </div>
            )}

            {showUserMenu && verified && email && (
              <div className="absolute bottom-full left-2 right-2 z-50 mb-1 overflow-hidden rounded-lg border border-white/10 bg-[#09090b] shadow-xl">
                ...
                <button
                  ...
                  className="flex w-full items-center gap-2 px-3 py-2 text-[13px] text-zinc-400 transition-colors hover:bg-white/5 hover:text-red-400"
                >
        ...
        >
          <div
            className={cn(
              "absolute right-0 top-0 h-full w-px bg-white/5 transition-all duration-100",
              isResizing ? "w-0.5 bg-white/20" : "group-hover:bg-white/10"
            )}
          />
        </div>
      </aside>
...
function NavItem({ icon, label, active, onClick, count, disabled }: NavItemProps) {
  return (
    <button
    ...
    >
      ...
      {count != null && (
        <span className="mr-2 flex-none rounded-full border border-white/10 px-2 py-0.5 text-[10px] tabular-nums leading-none text-zinc-400">
          {count}
        </span>
      )}
    </button>
